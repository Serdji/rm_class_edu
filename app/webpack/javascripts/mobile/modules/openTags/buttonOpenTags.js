...
module.exports = nodes => {
  ...
  each(nodes.hashTags, el => {
    // Проверка на наличие табов, если массив нодов пустой, останавливаем выполнение
    ...
    each(el.childNodes, child => {
      ...
      if (childTop > standardTop) arrNodes.push(child);
    });
    ...
    // Шаблон кнопки тега
    let tag      = `
      <div class='questions__point js-point-tags js-open-tags'>
        <span>+${sizeTag}</span>
      </div>
    `;
